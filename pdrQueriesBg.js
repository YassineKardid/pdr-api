...
const getStockByMagazinCrRgRest = (request, response) =>{
  q1 = "select ES.magazin , sum(ES.stock_ae*ES.pmp) as sum_dh_critique from excel_stock ES join "
  +"(select APPRO.code_article, APPRO.appro_type from approvisionnement APPRO group by APPRO.code_article, "
  +"APPRO.appro_type) APPROJ "
  +"on ES.article = APPROJ.code_article and APPROJ.appro_type = 'critique' and ES.magazin like 'G05%' group by ES.magazin";
  connection.query(q1, (error, results1)=>{ 
    if (error) { throw error}
    q2 = "select ES.magazin , sum(ES.stock_ae*ES.pmp) as sum_dh_reguliere from excel_stock ES join "
    +"(select APPRO.code_article, APPRO.appro_type from approvisionnement APPRO group by APPRO.code_article, "
    +"APPRO.appro_type) APPROJ "
    +"on ES.article = APPROJ.code_article and APPROJ.appro_type = 'reguliere' and ES.magazin like 'G05%' group by ES.magazin";
    connection.query(q2, (error, results2)=>{
      ...
      +"and ES.magazin like 'G05%' group by ES.magazin";
...
const getStockByMagazinNBRDist = (request, response) =>{
  ...
  q1 = "select ES.magazin , count(ES.article) as nbr_critique from excel_stock ES join "
  +"(select APPRO.code_article, APPRO.appro_type from approvisionnement APPRO group by APPRO.code_article,"
  +"APPRO.appro_type) APPROJ "
  +"on ES.article = APPROJ.code_article and APPROJ.appro_type = 'critique' and ES.magazin like 'G05%' group by ES.magazin";
  connection.query(q1, (error, results1)=>{
    if (error) { throw error}
    q2 = "select ES.magazin , count(ES.article) as nbr_reguliere from excel_stock ES join "
    +"(select APPRO.code_article, APPRO.appro_type from approvisionnement APPRO group by APPRO.code_article,"
    +"APPRO.appro_type) APPROJ "
    +"on ES.article = APPROJ.code_article and APPROJ.appro_type = 'reguliere' and ES.magazin like 'G05%' group by ES.magazin";
    connection.query(q2, (error, results2)=>{ 
...
const getStockPourcentageCrRgRest = (request, response)=>{
  q1 = "select sum(ES.stock_ae*ES.pmp) as sum_dh_critique from excel_stock ES join "
  +"(select APPRO.code_article, APPRO.appro_type from approvisionnement APPRO group by APPRO.code_article,"
  +"APPRO.appro_type) APPROJ "
  +"on  ES.article = APPROJ.code_article and APPROJ.appro_type = 'critique' and ES.magazin like 'G05%';"
  connection.query(q1, (error, results1)=>{
    if (error) { throw error}
    q2 = "select sum(ES.stock_ae*ES.pmp) as sum_dh_reguliere from excel_stock ES join "
    +"(select APPRO.code_article, APPRO.appro_type from approvisionnement APPRO group by APPRO.code_article,"
    +"APPRO.appro_type) APPROJ "
    +"on  ES.article = APPROJ.code_article and APPROJ.appro_type = 'reguliere' and ES.magazin like 'G05%';"
    connection.query(q2, (error, results2)=>{
      if (error) { throw error}
      q3 = "select sum(ES.stock_ae*ES.pmp) as sum_dh_rest from excel_stock ES "
      +"where ES.article not in (select distinct(APPRO.code_article) from approvisionnement APPRO)"
      +"and ES.magazin like 'G05%' ";
      connection.query(q3, (error, results3)=>{
        if (error) { throw error}
         vect = []
...
const getStockPourcentageCrRgRestNBR = (request, response)=>{
  q1 = "select count(ES.stock_ae) as sum_nbr_critique from excel_stock ES join "
  +"(select APPRO.code_article, APPRO.appro_type from approvisionnement APPRO group by APPRO.code_article,"
  +"APPRO.appro_type) APPROJ "
  +"on ES.article = APPROJ.code_article and APPROJ.appro_type = 'critique' and ES.magazin like 'G05%' ";
  connection.query(q1, (error, results1)=>{
    if (error) { throw error}
    q2 = "select count(ES.stock_ae) as sum_nbr_reguliere from excel_stock ES join "
    +"(select APPRO.code_article, APPRO.appro_type from approvisionnement APPRO group by APPRO.code_article,"
    +"APPRO.appro_type) APPROJ "
    +"on ES.article = APPROJ.code_article and APPROJ.appro_type = 'reguliere' and ES.magazin like 'G05%' "
    connection.query(q2, (error, results2)=>{
...
const getTotaleApproStockMinMaxReguliere = (request, response) =>{
  ...
  +"ACAPPROJ.code_article=ES.article where ES.magazin like 'G05%' group by ES.article,"
...
const getStockApproPourcentageReguliere = (request, response) =>{
  ...
  +"  where APPRO.appro_type='reguliere' "
  +" group by  APPRO.appro_type, APPRO.code_article, APPRO.stock_min, APPRO.stock_max, AC.fournisseur,"
  +"AC.numero , AC.date_debut, AC.date_fin "
  +") ACAPPROJ join excel_stock ES on "
  +"ACAPPROJ.code_article=ES.article where ES.magazin like 'G05%' group by ES.article,"
  +"ACAPPROJ.stock_min,ACAPPROJ.stock_max, ES.pmp,ES.description,ACAPPROJ.numero_ac,"
...
const getStockApproPourcentageCritique = (request, response) =>{
  ...
  +"  from approvisionnement APPRO "
  +"  join accord_cadre AC on AC.accord_cadre_id = APPRO.accord_cadre_id"
  +"  where APPRO.appro_type='critique' "
  +" group by  APPRO.appro_type, APPRO.code_article, APPRO.stock_min, APPRO.stock_max, AC.fournisseur,"
  +"AC.numero , AC.date_debut, AC.date_fin "
  +") ACAPPROJ join excel_stock ES on "
  +"ACAPPROJ.code_article=ES.article where ES.magazin like 'G05%' group by ES.article,"
...
const getStockByMagazinDH = (request, response) =>{
  ...
  +"approvisionnement APPRO) and ES.magazin like 'G05%' group by ES.magazin, ES.nature";
...
const getStockByMagazinNBR = (request, response) =>{
  ...
  +"from approvisionnement APPRO) and ES.magazin like 'G05%' group by ES.magazin, ES.nature"
  connection.query(sql, (error, results)=>{
    if (error) { throw error}
    vect = []  
    for(i=0;i<results.length;i++){
