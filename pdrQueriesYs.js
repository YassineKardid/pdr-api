...
const getStockByMagazinCrRgRest = (request, response) =>{
  ...
  +"on ES.article = APPROJ.code_article and APPROJ.appro_type = 'critique' and ES.magazin like 'G09%' group by ES.magazin";
  connection.query(q1, (error, results1)=>{ 
    ...
    +"(select APPRO.code_article, APPRO.appro_type from approvisionnement APPRO group by APPRO.code_article, "
    +"APPRO.appro_type) APPROJ "
    +"on ES.article = APPROJ.code_article and APPROJ.appro_type = 'reguliere' and ES.magazin like 'G09%' group by ES.magazin";
...
const getStockPourcentageCrRgRest = (request, response)=>{
  ...
  +"(select APPRO.code_article, APPRO.appro_type from approvisionnement APPRO group by APPRO.code_article,"
  +"APPRO.appro_type) APPROJ "
  +"on  ES.article = APPROJ.code_article and APPROJ.appro_type = 'critique' and ES.magazin like 'G09%';"
  connection.query(q1, (error, results1)=>{
    if (error) { throw error}
    q2 = "select sum(ES.stock_ae*ES.pmp) as sum_dh_reguliere from excel_stock ES join "
    +"(select APPRO.code_article, APPRO.appro_type from approvisionnement APPRO group by APPRO.code_article,"
    +"APPRO.appro_type) APPROJ "
    +"on  ES.article = APPROJ.code_article and APPROJ.appro_type = 'reguliere' and ES.magazin like 'G09%';"
    connection.query(q2, (error, results2)=>{
      if (error) { throw error}
